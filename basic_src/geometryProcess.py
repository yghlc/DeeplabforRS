...
def setGCPsfromptsFile(imagefile, projection, GeoTransform, ptsfile):
    ...
    Outputtiff = io_function.get_name_by_adding_tail(imagefile,'new') # imagefile.split('.')[0] + '_new.tif'
    format = "GTiff"
    ...
    for bandindex in range(0, image.GetBandCount()):
        ...
        banddata = bandobject.ReadRaster(0, 0, image.GetWidth(), image.GetHeight(), image.GetWidth(), image.GetHeight(),
                                         datatype)
        # byte
        # if banddata is 1:
        #     bandarray = struct.unpack('B'*image.GetWidth()*image.GetHeight(), banddata)
        dst_ds.GetRasterBand(bandindex + 1).WriteRaster(0, 0, image.GetWidth(), image.GetHeight(), banddata,
...
def output_tie_points_vector_on_base_image(drawed_image, tiepoint_rms_file, outputfile):
    ...
    for k in range(0, len(x0)):
        test_aggdraw.line(draw_content, (x0[k], y0[k], x1[k], y1[k]), color='red', width=1, arrow='last',
                          arrowshape=(4, 5, 3))
...
def coregistration_siftGPU(basefile, warpfile, bkeepmidfile, xml_obj):
    ...
    try:
        ...
        try:
            ...
            if totalrms_value > acceptable_rms:
                basic.outputlogMessage(
                    "ERROR:Total RMS(%f) exceeds the acceptable one(%f)" % (totalrms_value, acceptable_rms))
                return False
        except ValueError:
    ...
    # warp image
    warpresultfile = io_function.get_name_by_adding_tail(Outputtiff,'warp') # Outputtiff.split('.')[0] + '_warp.tif'
    # -order 1  -tps
    ...
    if order_number is False:
        return False
    CommandString = 'gdalwarp ' + ' -order ' + str(order_number) + ' -r bilinear -tr ' + str(xres) + ' ' + str(
